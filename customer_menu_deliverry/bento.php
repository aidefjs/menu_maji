...
<body class="bg-white">
    ...
    <div>
            ...
            <div class="flex justify-center p-5 pb-5 px-10 items-center pb-2 mb-4 mt-4">
                <div class="flex space-x-4 items-center">
                    <div class="flex items-center justify-center bg-gray-50 rounded-3xl hidden sm:flex">
                        <div>
                            <button id="delivery-button" class="flex px-4 py-2 rounded-3xl text-red-700 font-medium text-sm bg-red-500 text-white">
                                ✔ จัดส่งถึงบ้าน
                            </button>
                        </div>
                        <button id="pickup-button" class="rounded-3xl px-4 py-2 rounded text-red-700 font-medium text-sm">
                            ✔ รับที่ร้าน
                        </button>
                    </div>
        ...
        <div id="dropdownmenu" class="flex justify-center items-center z-50 hidden bg-gray-50 rounded-3xl shadow w-44 mb-2 mt-[-10px] max-[640px]:mx-auto max-[640px]:mt-[-20px] mx-auto min-[640px]:mx-[49.5%] min-[640px]:mt-[-20px]">
            <ul class="py-2 text-gray-700 " aria-labelledby="dropdownDefaultButton">
                ...
                <li class="">
                    <button onclick="getLocation()" class="flex items-center px-4 py-2 text-black rounded-3xl text-red-600 font-bold hover:bg-gray-200 w-44 " style="font-size: 14px;">
    ...
    <div class="grid w-full h-screen sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-4">
        <div class="md:col-span-3 sm:col-span-4 items-center justify-center">
            ...
            $sql = "SELECT * FROM menu WHERE menuTypeID = 3 ;";
            $result = mysqli_query($conn, $sql);
            echo '<form  method="get" class="grid p-20 sm:grid-cols-2 lg:grid-cols-3  gap-1 mt-[-7%]"> ';

            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    echo '<div class="justify-center menu-item w-[75%] h-70 mx-auto ext-pretty  bg-white border border-gray-200 rounded-xl">
                        <img class="h-30 object-cover mb-2" src="' . $row["menuImage"] . '" alt="' . $row["menuName"] . '">
                        <p class="text-black text-pretty ml-[5%] mr-[5%] font-bold">' . $row["menuName"] . '</p>
                        <p class="text-gray-400 text-sm text-pretty ">' . $row["menuDescription"] . '</p>
                        <p class="text-black mt-2 ml-[5%] mr-[5%]">' . '฿' . $row["menuPrice"] . '.-</p>
                        <button type="submit" name="order_menu" value="Submit" class="mt-2 text-white bg-red-500 hover:bg-yellow-500 font-medium rounded-full text-sm px-5 py-2.5 text-center">
                            สั่งอาหาร
                        </button>
                    </div>';
                }
    ...
    <div id="default-modal" tabindex="-1" aria-hidden="true" class=" hidden  fixed  z-50 justify-center items-center md:inset-0 h-screen w-[30%] mx-auto flex justify-center items-center selfs-center">
        <div class="relative p-4 w-full max-w-2xl h-screen">
            <div class="relative rounded-3xl shadow-xl shadow-white-300/40">
                <div class="w-full mx-auto bg-black p-8 rounded-md shadow-md m-2">
                    <h2 class="text-red-500 text-2xl font-semibold mb-6">ข้อมูลติดต่อ</h2>

                    <form action="process_form.php" method="post">
                        <div class="mb-4">
                            <label for="name" class="block text-sm font-medium text-gray-100">ชื่อที่อยู่ :</label>
                            <input type="text" id="name" name="name" class="mt-1 p-2 w-full border rounded-md">
                        </div>

                        <div class="mb-4">
                            <label for="phone" class="block text-sm font-medium text-gray-100">เบอร์ติดต่อ :</label>
                            <input type="tel" id="phone" name="phone" class="mt-1 p-2 w-full border rounded-md">
                        </div>

                        <div class="mb-4">
                            <label for="address" class="block text-sm font-medium text-gray-100">ที่อยู่ :</label>
                            <input type="tel" id="address" name="address" class="mt-1 p-2 w-full border rounded-md">
                        </div>

                        <div class="p-4 flex justify-center items-center ">
                            <button data-modal-hide="default-modal" type="button" class="text-white bg-blue-700 hover:bg-red-600 font-medium rounded-lg text-sm px-5 py-2.5 text-center bg-red-400 mr-4">ยืนยัน</button>
                            <button data-modal-hide="default-modal" type="button" onclick="toggleModal('default-modal')" class="text-white bg-blue-700 hover:bg-red-600 font-medium rounded-lg text-sm px-5 py-2.5 text-center bg-red-400">ยกเลิก</button>
